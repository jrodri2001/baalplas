...
function optionsframework_add_admin() {
    ...
    $of_page = add_theme_page( THEMENAME, 'Opciones BaalPlas', 'edit_theme_options', 'optionsframework', 'optionsframework_options_page');
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$pulsar_data = get_option(BACKUPS);
		
		update_option(OPTIONS, $pulsar_data);
		
		die('1'); 
	}
	elseif($save_type == 'import_options'){
			
		$pulsar_data = $_POST['data'];
		$pulsar_data = unserialize(base64_decode($pulsar_data)); //100% safe - ignore theme check nag
		update_option(OPTIONS, $pulsar_data);
	...
	{
		wp_parse_str(stripslashes($_POST['data']), $pulsar_data);
		unset($pulsar_data['security']);
		unset($pulsar_data['of_save']);
		update_option(OPTIONS, $pulsar_data);
