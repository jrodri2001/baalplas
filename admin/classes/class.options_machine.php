...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
	    ...
	    $pulsar_data = get_option(OPTIONS);
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($pulsar_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected($pulsar_data[$value['id']], $option, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($pulsar_data[$value['id']]);			
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($pulsar_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($pulsar_data[$value['id']])) {
						$pulsar_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($pulsar_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					(isset($pulsar_data[$value['id']]))? $multi_stored = $pulsar_data[$value['id']] : $multi_stored="";
				...
				//colorpicker option
				case 'color':		
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$pulsar_data[$value['id']].'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $pulsar_data[$value['id']] .'" />';
				break;
				
				//typography option	
				case 'typography':
				
					$typography_stored = isset($pulsar_data[$value['id']]) ? $pulsar_data[$value['id']] : $value['std'];
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $pulsar_data[$value['id']];
				...
				case 'images':
					...
					$select_value = $pulsar_data[$value['id']];
				...
				case 'sorter':
					...
					$sortlists = isset($pulsar_data[$value['id']]) && !empty($pulsar_data[$value['id']]) ? $pulsar_data[$value['id']] : $value['std'];
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($pulsar_data[$value['id']]) && !empty($pulsar_data[$value['id']]) ? $pulsar_data[$value['id']] : '';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','pulsar' ).'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($pulsar_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
				...
				case 'select_google_font':
					...
					foreach ($value['options'] as $select_key => $option) {      
						$output .= '<option value="'.$select_key.'" ' . selected((isset($pulsar_data[$value['id']]))? $pulsar_data[$value['id']] : "", $option, false) . ' />'.$option.'</option>';   
					} 
					$output .= '</select></div>';
